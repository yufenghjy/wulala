...
//char move[100][100];
char start;
char final[100];//最终状态的集合
int len_final; //最终状态的数量
bool Is_final[150];
...
NFA_set mymove[100][100]; //从某个状态通过某个输入符号可以到达的所有状态的集合
...
void createNFA()
{
	...
	input.open("D:\\cwork\\lab\\lab\\词法分析_文法.txt");
	input >> N;
	while (N--)
	{
		...
		if (strlen(str) > 1)
		{
			mymove[ch][str[0]].set[mymove[ch][str[0]].len++] = str[1];
		}
		else
		{
			//if(str[0]=='@')        //用@表示ε
				//is_final[ch]=true;
			//else
			mymove[ch][str[0]].set[mymove[ch][str[0]].len++] = 'Y';  //终态
		}
	}
}
void showNFA()
{
	for (int i = 0; i < 100; ++i)
	{
		for (int j = 0; j < 100; ++j)
		{
			...
			{
				if (mymove[i][j].set[k] != '#')
					cout << char(i) << " " << char(j) << " " << k << " " << mymove[i][j].set[k] << endl;
			}
		}
	}
}

//用于查找给定状态集合在DFA状态表中的索引
...
void get_closure(NFA_set& temp)    //得到一个完整的子集 
{
	for (int i = 0; i < temp.len; ++i)
	{
		for (int j = 0; j < mymove[temp.set[i]]['@'].len; ++j)
		{
			if (!Is_in_set(mymove[temp.set[i]]['@'].set[j], temp))
			{
				temp.set[temp.len++] = mymove[temp.set[i]]['@'].set[j];
			}
		}
	}
}
...
{
	num_new_set = 0;
	NFA_set work_set;//当前正在处理的状态集合
	NFA_set worked_set;//处理过程中生成的新的状态集合
	...
	while (!s.empty())
	{
		...
		for (int i = 0; i < len_final; ++i)
		{
			...
			{
				for (int k = 0; k < mymove[work_set.set[j]][final[i]].len; ++k)
				{
					//如果转移后有结果，且不为终态，不在已有的状态集中，则添加进去
					if (mymove[work_set.set[j]][final[i]].set[k] != '#' && mymove[work_set.set[j]][final[i]].set[k] != 'Y' && !Is_in_set(mymove[work_set.set[j]][final[i]].set[k], worked_set))
					{
						worked_set.set[worked_set.len++] = mymove[work_set.set[j]][final[i]].set[k];
					}
					//如果转移后是终态，且不在已有的状态集中，则添加该集合为终态集
					if (mymove[work_set.set[j]][final[i]].set[k] == 'Y' && !Is_in_set(mymove[work_set.set[j]][final[i]].set[k], worked_set))
					{
...
int main()
{
	...
	output.open("D:\\cwork\\lab\\lab\\output.txt");
	scan();
	fclose(file_source);
	output.close();
	return 0;
